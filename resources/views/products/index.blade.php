    ...
    <x-slot name="header">
        <div class="flex items-center ">
            <button
                ...
                class="bg-red-400 dark:bg-blue-900 hover:bg-red-300 text-white dark:text-white dark:hover:bg-blue-700 py-2 px-4 rounded">
                Tambah Produk
            </button>
        </div>
    </x-slot>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        @foreach ($products as $product)
                            <div class="border rounded-lg shadow-md p-3 bg-red-300 dark:bg-gray-700">
                            <img src="{{ $product->image_url }}" alt="{{ $product->name }}" class="w-full h-52 object-cover mb-4">
                                ...
                                <div class="flex items-center space-x-2 ">
                                    ...
                                    <x-danger-button x-data=""
                                        ...
                                        x-on:click="$dispatch('set-action', '{{ route('product.destroy', $product->id) }}')"  class="py-4 px-4 text-sm mr-5">
                                        {{ __('Hapus') }}
                                    </x-danger-button>
                                </div>
                            </div>
